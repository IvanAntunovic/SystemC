...
SC_MODULE(mem_unit) {
  ...
  mem_unit(sc_module_name name)
      : sc_module(name),
        /* construct the peq and register the callback */
        m_request_peq("request_peq",this,&mem_unit::request_queue_callback)
  {
    // register TLM callback methods
    tgt_socket.register_nb_transport_fw(this, &mem_unit::nb_transport_fw);

    // copy initial memory contents
    int init_data_size = sizeof(MEM_INIT_DATA)/sizeof(int);
    for (int addr = 1; addr <= init_data_size; addr++) {
    storage[addr] = MEM_INIT_DATA[addr-1];
    }
    storage[0] = init_data_size;
    m_in_request = false;
    m_in_response = false;
  ...
  tlm_utils::simple_target_socket<mem_unit, 32,
          tlm::tlm_base_protocol_types> tgt_socket;

  int storage[8192];

  // use a PEQ to store incoming requests
  peq_with_cb_and_phase<mem_unit>	m_request_peq;	

  // callback of the PEQ
  void request_queue_callback(tlm_base_protocol_types::tlm_payload_type& trans,
    const tlm_base_protocol_types::tlm_phase_type& stored_phase);

  // can not accept a request until END_REQ is sent
  bool m_in_request;

  // can not send a response until END_RESP is received
  bool m_in_response;


}; // END SC_MODULE "mem_unit"
